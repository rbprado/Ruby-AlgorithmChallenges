...
def dirtyChooser(bot, dirty)
    ...
    STDERR.puts '--- dirty'
    STDERR.puts dirty.inspect
    STDERR.puts '--- dirty_dist'
    STDERR.puts dirty_dist.inspect
    STDERR.puts '--- abs_dist'
    STDERR.puts abs_dist.inspect
    STDERR.puts '--- abs_dist.min'
    STDERR.puts abs_dist.min.inspect
    STDERR.puts '--- abs_dist.index(abs_dist.min)'
    STDERR.puts abs_dist.index(abs_dist.min).inspect
    STDERR.puts '--- dirty_dist.at(abs_dist.index(abs_dist.min))'
    STDERR.puts dirty_dist.at(abs_dist.index(abs_dist.min))
